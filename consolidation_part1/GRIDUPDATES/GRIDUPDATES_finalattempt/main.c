...
int firstnumcheck (int numberinput)
{
    int firstnumber1;

    while(checkrange(numberinput)==true) //grid will not be printed if more than 100
    {
...
int main()
{
    ...
    scanf("%d", &firstnumber);
    firstnumber = firstnumcheck(firstnumber);
    ...
    //can further be improved with a whole new function for the while loop (possible to read in input?)
    //has to be a int type function with return
